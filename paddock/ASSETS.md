...
`images/minigames/paddock/assets/` are 3D models (GLB/FBX/OBJ) for an engine this
site does not have. They were left alone.

The sheets live in `images/minigames/paddock/reference/` (gitignored) and the
scripts that cut them are in `notes/paddock-assets/`. Re-run either from the repo
root; they write straight into `paddock/assets/` (26 files, ~900K).
