...
(() => {
  ...
  const SAVE_KEY = 'blackout-save-v2';
  const OLD_SAVE_KEY = 'spy-assassin-save-v1';
  const SOUND_KEY = 'blackout-sound';
  const DIFF_KEY = 'blackout-difficulty';
  const SCALE_KEY = 'blackout-scale';
  // The game lived in spy-assassin/ until 2026-09-24; carry its saved keys across once.
  [[SAVE_KEY, 'spy-assassin-save-v2'], [SOUND_KEY, 'spy-assassin-sound'], [DIFF_KEY, 'spy-assassin-difficulty'], [SCALE_KEY, 'spy-assassin-scale']].forEach(([now, was]) => {
    try {
  ...
  function freshGuards() {
    ...
    return [
      ...
      // is the one he actually walks (test/blackout/beats.js holds him to it)
