/* Image Studio — drop in a photo and restyle it. The page: loading the image,
   the layer stacks for Both, Foreground and Background, presets, the live
   preview and the download. The styles themselves are in filters.js and the
   foreground/background split in background.js. */
(function () {
  'use strict';

  // The studio lived in image-filters/ until 2026-09-24; carry its saved keys across once.
  [['image-studio-ui-v1', 'image-filters-ui-v1'], ['image-studio-presets-v1', 'image-filters-presets-v1']].forEach(([now, was]) => {
    try {
  ...
  function computeMask() {
    ...
    try {
      ...
      state.bgMask = null;   // no mask: the photo is treated as one region
      state.regionW = null;
      state.cover = null;
      state.labels = null;
    }
  ...
  const UI_KEY = 'image-studio-ui-v1';
  ...
  const PRESET_KEY = 'image-studio-presets-v1';
  ...
  function saveUserPresets() {
    ...
    } catch (err) { /* storage full or blocked: the presets last until the tab closes */ }
  }
    ...
    requestAnimationFrame(() =>
      requestAnimationFrame(() => {
        ...
        try {
          ...
          drawStage();
        } catch (err) { /* a failed render keeps the previous result on screen */ }
