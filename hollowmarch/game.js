/* Hollowmarch — a village at a crossroads, and a keep at the end of the roads.
   Build between waves: houses and farms for gold, walls and towers along the
   roads. Then a wave comes down the roads, and the keep has to still be standing
   when it is over. */
(() => {
  'use strict';
  ...
  const W = 8;
  const H = 8;
  const SAVE_KEY = 'hollowmarch-save-v1';
  // The game lived in crossroads-inn/ until 2026-09-24; carry its saved keys across once.
  [[SAVE_KEY, 'crossroads-keep-v1']].forEach(([now, was]) => {
    try {
